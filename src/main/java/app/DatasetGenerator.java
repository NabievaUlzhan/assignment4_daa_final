...
public class DatasetGenerator {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Datasets generated into /data");
    }

    private static void makeDataset(
            File dataDir, String fileName, int n, boolean directed, int source,
            String model, Random rnd, double density, boolean allowCycles) throws Exception {

        System.out.println("→ Generating " + fileName + " (n=" + n + ", density=" + density +
        ...
        int attempts = 0;
        int attemptsCap = Math.max(10000, target * 50);
        while (edges.size() < target && attempts < attemptsCap) {
            attempts++;
            int u = rnd.nextInt(n);
            int v = rnd.nextInt(n);
            if (u == v) continue;
        ...
        if (allowCycles && n >= 6) {
            addIfMissing(edges, 2, 1, 1);
            addIfMissing(edges, 3, 2, 1);
            addIfMissing(edges, 1, 3, 1);
            addIfMissing(edges, 6 % n, 5 % n, 2);
        }
        ...
        System.out.println("   wrote " + fileName + " with " + edges.size() + " edges");
    }

    private static void addIfMissing(List<EdgeData> edges, int u, int v, int w) {
        for (int k = 0; k < edges.size(); k++) {
            EdgeData e = edges.get(k);
            if (e.getU() == u && e.getV() == v) return;
        }
        edges.add(new EdgeData(u, v, w));
    }
}
